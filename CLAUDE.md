...
There is no test target yet (see `Documentation/ROADMAP.md` — testing is a planned
milestone). When one exists, run a single test with
`xcodebuild test -only-testing:GusTests/<Suite>/<test> ...`.
...
A **muted Psych green** background (a desaturated, darker take on the show's lime — not
the brownish forest tone) with a **muted yellow** (not pastel — desaturated but clearly
yellow/ochre) **pineapple**: an egg-shaped body carrying the classic diamond crosshatch
lattice, topped by a tight, mostly upright spiky crown (bases clustered to ~1/4 of the top,
tips spreading to ~1/3 of the width). Current values:

| Role | Hex |
|---|---|
| Background — muted Psych green | `#3F5A25` |
| Pineapple — muted yellow/ochre | `#C7A23C` |

The icon's yellow is a more muted sibling of the app `AccentColor` (pineapple gold
`#F4B740`) so brand and chrome stay coherent. A single multiplatform asset catalog needs
the icon present as `AppIcon.appiconset` (iOS + macOS idioms), `AppIcon.brandassets`
(tvOS), and `AppIcon.solidimagestack` (visionOS) — all named `AppIcon` — with
`ASSETCATALOG_COMPILER_APPICON_NAME` resolving to `AppIcon` (no longer overridden empty).
The placeholder art is generated by a small Core Graphics renderer (Apple-first, no
...
branch and re-expressed on `@Observable` with a Psych palette.
