...
Earthy, darker **green** background with a **muted yellow** (not pastel — desaturated but
clearly yellow/ochre) **pineapple silhouette**. Starting values to tune:

| Role | Hex |
|---|---|
| Background — earthy dark green | `#2E3A24` |
| Pineapple silhouette — muted yellow | `#C7A23C` |
...
(tvOS), and `AppIcon.solidimagestack` (visionOS) — all named `AppIcon` — and the
`ASSETCATALOG_COMPILER_APPICON_NAME: ""` override removed from `project.yml`. See the
roadmap's Brand milestone.
