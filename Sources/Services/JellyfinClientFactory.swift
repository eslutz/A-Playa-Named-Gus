...
/// wiring in A Playa Named Gus's device identity and a shared, image-tuned `URLCache`.
///
/// Pattern reference: Swiftfin's `UserSession` / `ConnectToServerViewModel` construct a
/// `JellyfinClient.Configuration` the same way; A Playa Named Gus centralizes it here.
