...
| Role | Hex |
|---|---|
| Background — Jellyfin navy | `#000B25` |
| Pineapple gradient start — Jellyfin purple | `#AC5CC3` |
| Pineapple gradient end / fill light — Jellyfin blue | `#00A4DC` |

The same purple/blue pair drives `AccentColor`, cinema key/fill lights, and launch
background. A single multiplatform asset catalog needs the icon present as
`AppIcon.appiconset` (iOS + macOS idioms), `AppIcon.brandassets` (tvOS), and
`AppIcon.solidimagestack` (visionOS) — all named `AppIcon` — with
`ASSETCATALOG_COMPILER_APPICON_NAME` resolving to `AppIcon`. The placeholder art is
generated by `Scripts/generate-app-icon.swift` using Core Graphics (Apple-first, no
